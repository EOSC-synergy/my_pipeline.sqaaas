...
EOF
printf "$(cat auth.dat)" "${IM_USER}" "${IM_PASS}" "${OPENSTACK_USER}" "${OPENSTACK_PASS}" > auth.dat
...
echo
im_client.py -r "https://appsgrycap.i3m.upv.es:31443/im/" -a "auth.dat" create_wait_outputs /im/test-ost.tosca > ./im_tosca.json
RETURN_CODE=$?
...
if [ ${RETURN_CODE} -eq 0 ] && ! [[ -z "${INFID}" && "x${INFID}x" == "xnullx" ]]; then
  ...
  im_client.py -r "https://appsgrycap.i3m.upv.es:31443/im/" -a "auth.dat" getcontmsg ${INFID}
  echo
  im_client.py -r "https://appsgrycap.i3m.upv.es:31443/im/" -a "auth.dat" destroy ${INFID}
